    ...
    function handleMessage(message){
        switch(message.method){
			...
			case 'mining.extranonce.subscribe':
				if (options.coin.algorithm === 'verus' || options.coin.algorithm === 'equihash')
                	sendJson({
						id: message.id,
						result: false,
						error: [20, "Not supported.", null]
					});
				else _this.emit('unknownStratumMethod', message);
                break;
            default:
    ...
    function handleSubscribe(message){
        ...
        _this.emit('subscription',
            ...
            function(error, extraNonce1, extraNonce2Size){
                ...
                sendJson({
                    id: message.id,
                    result: (options.coin.algorithm === 'verus' || options.coin.algorithm === 'equihash')?
						[
							null, //sessionId
							extraNonce1
						], : 
        ...
        _this.emit('submit',
            {
                ...
                extraNonce2 : message.params[(options.coin.algorithm === 'verus' || options.coin.algorithm === 'equihash')?3:2],
                nTime       : message.params[(options.coin.algorithm === 'verus' || options.coin.algorithm === 'equihash')?2:3],
                nonce       : message.params[(options.coin.algorithm === 'verus' || options.coin.algorithm === 'equihash')?_this.extraNonce1 + message.params[3]:4],
				soln		: (options.coin.algorithm === 'verus' || options.coin.algorithm === 'equihash')? message.params[4] : undefined,
            },
