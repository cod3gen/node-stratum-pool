...
var pool = module.exports = function pool(options, authorizeFn) {
    ...
    function StartStratumServer(finishedCallback) {
        ...
        _this.stratumServer.on('started', function () {
            ...
            client.on('difficultyChanged', function (diff) {
                ...
                resultCallback(null,
                    extraNonce,
                    (options.coin.algorithm === 'verus' || options.coin.algorithm === 'equihash')? extraNonce : _this.jobManager.extraNonce2Size
                );
    ...
    function GetBlockTemplate(callback){
		...
		if (options.coin.algorithm === 'verus' || options.coin.algorithm === 'equihash') getBlockSubsidyandTemplate();
        else getBlockTemplate(undefined);
