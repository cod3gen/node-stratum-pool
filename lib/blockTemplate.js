...
var BlockTemplate = module.exports = function BlockTemplate(
	...
	let blockReward = {
		...
		'coinbase': rpcData.coinbasevalue
    };
    ...
    this.serializeHeader = function(merkleRoot, nTime, nonce){ // Changed!
		...
		header.write(this.isZ ? this.prevHashReversed, position += 4, 32, 'hex' : this.rpcData.bits, position += 4, 4, 'hex');
		header.write(this.isZ ? this.merkleRootReversed, position += 32, 32, 'hex' : nTime, position += 4, 4, 'hex');
		header.write(this.isZ ? this.hashReserved, position += 32, 32, 'hex' : merkleRoot, position += 4, 32, 'hex');
		header.write(this.isZ ? nTime, position += 32, 4, 'hex' : this.rpcData.previousblockhash, position += 32, 32, 'hex');
