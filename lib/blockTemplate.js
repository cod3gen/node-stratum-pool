...
var BlockTemplate = module.exports = function BlockTemplate(
	...
	if (typeof this.genTx === 'undefined') {
		...
		if (this.isZ) this.genTxHash = transactions.txHash();
    }
		
	this.merkleRoot = this.isZ ? merkle.getRoot(rpcData, this.genTxHash) : null;
	this.txCount = this.rpcData.transactions.length + 1; // add total txs and new coinbase
    this.merkleRootReversed = this.isZ ? util.reverseBuffer(new Buffer(this.merkleRoot, 'hex')).toString('hex') : null;
    ...
    this.serializeHeader = function(merkleRoot, nTime, nonce){ // Changed!
		...
		header.write(this.isZ ? this.hashReserved : this.merkleRoot, this.isZ ? position += 32 : position += 4, 32, 'hex');
